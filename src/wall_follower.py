...
class WallFollower:
    ...
    L1 = DESIRED_DISTANCE + 0.3 # Look ahead distance for pure pursuit controller 1.3 works
    max_thresh = 5 # remove distant wall noise from lidar 0.7 works
    min_thresh = 0.1 # remove car noise from lidar
    front = (0.5,0.15) #offset and split of front of car detection for safety controller
    right = (0.0,0.60) # start and end fraction for selection of lidar data for right side of car (0.3,0.55) works
    ...
    s_w = 0.65 # width of safety space 0.5 default wider than that is safer
    ...
    def __init__(self):
        ...
        self.err_pub = rospy.Publisher('Cur_Error', Float32, queue_size=1)
        self.count = 0
        self.Moving = True
        self.toterr = 0
        self.turn_msg = AckermannDriveStamped()
        rospy.Subscriber(self.SCAN_TOPIC, LaserScan, self.callback, (self.pub, self.line_pub, self.turn_msg))
    ...
    def callback(self, msg, pub_list):
        ...
        if stop:
            turn_msg.drive.speed = -0.5*(self.Moving)
            self.Moving = False
        else:
            self.Moving = True
    ...
    def safety(self, ranges, angles):
        angle = self.turn_msg.drive.steering_angle
        neg = 1 if angle > 0 else -1
        next_ang = neg*max((abs(angle) + 0.05),0.34)
        tr = self.w_b/np.tan(next_ang)
        sw = self.s_w
        s = -self.SIDE
